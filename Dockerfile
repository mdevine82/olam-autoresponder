...
RUN  apt-get install -y --no-install-recommends dos2unix ntp make apache2 php7.2 libapache2-mod-php7.2 \
	php-memcached php7.2-curl php7.2-dev php7.2-gd php7.2-mysql php7.2-mbstring \
	php7.2-imap php-xdebug php7.2-zip\
...
RUN echo 'sendmail_path = /usr/bin/mhsendmail --smtp-addr smtp:1025' >> /etc/php/7.2/cli/php.ini
RUN echo 'sendmail_path = /usr/bin/mhsendmail --smtp-addr smtp:1025' >> /etc/php/7.2/apache2/php.ini
...
ADD ./.docker/php/xdebug.ini /etc/php/7.2/mods-available/xdebug.ini
...
RUN echo "xdebug.remote_host=172.20.0.1" >> /etc/php/7.2/mods-available/xdebug.ini
